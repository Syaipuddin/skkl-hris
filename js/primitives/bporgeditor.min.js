/*
 Basic Primitives orgEditor Demo v1.0.38
...
primitives.orgeditor.UserTemplateContact.prototype.onRender=function(a,c,b){var a=c.context,d,e,f,h,g;g=c.element.find("[name=readmorelabel]");switch(c.renderingMode){case primitives.common.RenderingMode.Create:g.click(function(a){primitives.common.stopPropagation(a)})}c.element.find("[name=photo]").attr({src:a.image});c.element.find("[name=titleBackground]").css({background:a.itemTitleColor});c.element.find("[name=title]").css({color:primitives.common.highestContrast(a.itemTitleColor,b.itemTitleSecondFontColor,
b.itemTitleFirstFontColor)});e=["title","description","phone","email","readmorelabel"];b=0;for(d=e.length;b<d;b+=1)f=e[b],h=c.element.find("[name="+f+"]"),f=null!=a[f]?a[f]:"",h.text()!==f&&h.text(f);g.css({visibility:!primitives.common.isNullOrEmpty(a.readmorelabel)?"inherit":"hidden"});primitives.common.isNullOrEmpty(a.readmoreurl)?g.removeAttr("href"):g.attr({href:a.readmoreurl})};primitives.orgeditor.UserTemplateDescription=function(){this.name="plainDescriptionTemplate"};
...
primitives.orgeditor.UserTemplateDescription.prototype.onRender=function(a,c,b){var a=c.context,d,e,f,h,g;g=c.element.find("[name=readmorelabel]");switch(c.renderingMode){case primitives.common.RenderingMode.Create:g.click(function(a){primitives.common.stopPropagation(a)})}c.element.find("[name=titleBackground]").css({background:a.itemTitleColor});c.element.find("[name=title]").css({color:primitives.common.highestContrast(a.itemTitleColor,b.itemTitleSecondFontColor,b.itemTitleFirstFontColor)});e=
["title","description","readmorelabel"];b=0;for(d=e.length;b<d;b+=1)f=e[b],h=c.element.find("[name="+f+"]"),f=null!=a[f]?a[f]:"",h.text()!==f&&h.text(f);g.css({visibility:!primitives.common.isNullOrEmpty(a.readmorelabel)?"inherit":"hidden"});primitives.common.isNullOrEmpty(a.readmoreurl)?g.removeAttr("href"):g.attr({href:a.readmoreurl})};primitives.orgeditor.UserTemplateInvisible=function(){this.name="invisibleTemplate"};primitives.orgeditor.UserTemplateInvisible.prototype=new primitives.orgeditor.UserTemplate;
...
primitives.orgeditor.Controller=function(){this.widgetEventPrefix="bporgeditor";this.options=new primitives.orgeditor.Config;this.selectedItem=this.dlgMessage=this.dlgCodeMirror=this.dlgConfig=this.dlgItemConfig=this.dlgOrgChart=this.orgDiagram=this.formsPlaceholder=null;this.fieldsToClone="pageFitMode orientationType verticalAlignment horizontalAlignment connectorType minimalVisibility selectionPathMode leavesPlacementType childrenPlacementType normalLevelShift dotLevelShift lineLevelShift normalItemsInterval dotItemsInterval lineItemsInterval itemTitleFirstFontColor itemTitleSecondFontColor defaultTemplateName showLabels labelOrientation labelPlacement labelSize items".split(" ");this.userTemplates=
...
primitives.orgeditor.Controller.prototype._updateItemConfigs=function(a){var c,b,d;c=0;for(b=a.length;c<b;c+=1)d=a[c],d.isVisible=!0,"invisibleTemplate"===d.templateName&&(d.isVisible=this.options.editMode)};primitives.orgeditor.Controller.prototype._updateLayout=function(){var a=new primitives.common.Rect(0,0,this.element.innerWidth(),this.element.innerHeight());this.formsPlaceholder.css(a.getCSS());this.orgDiagram.css(a.getCSS())};
...
primitives.orgeditor.Controller.prototype._onButtonClick=function(a,c){var b=this,d,e,f,h,g,i,j;switch(c.name){case "delete":null===c.parentItem?b.dlgMessage.dialog({modal:!0,buttons:{Ok:function(){jQuery(this).dialog("close")}}}):b.dlgConfirm.dialog({resizable:!1,height:240,modal:!0,buttons:{Delete:function(){j=b._getSubItemsForParent(c.context);j[c.context.id]=!0;i=[];e=0;for(f=b.options.items.length;e<f;e+=1)g=b.options.items[e],j.hasOwnProperty(g.id)||i.push(g);b.options.items=i;b.orgDiagram.orgDiagram("option",
...
{cancel:function(){},update:function(a,d){c.context.parent=d;b.orgDiagram.orgDiagram("update",primitives.common.UpdateMode.Refresh);b._trigger("onSave")}});d=new primitives.orgdiagram.Config;e=0;for(f=b.fieldsToClone.length;e<f;e+=1)h=b.fieldsToClone[e],d[h]=b.options[h];d.hasButtons=primitives.common.Enabled.False;d.hasSelectorCheckbox=primitives.common.Enabled.False;d.items=b.options.items;d.cursorItem=b.orgDiagram.orgDiagram("option","cursorItem");b.dlgOrgChart.bpDlgOrgDiagram("open",d,c.context)}}};
