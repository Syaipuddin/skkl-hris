	...
	$isSAPConnect = true;
		if (isset($_SESSION['auth_hrportal']) && $_SESSION['auth_hrportal']==1) {
			...
			if (isset($_SESSION['token']) && isset($_SESSION['nik'])) {
				...
				/*var_dump($config['auth-server-url'] . "/realms/".$usrpwd['realm']."/protocol/openid-connect/token/introspect", $_SESSION['token'], $usrpwd['backend-usrpwd']);
				echo $_SESSION['token'];*/
			    ...
			    // echo "<script>console.log('".$active."');</script>";
					...
					var_dump($active);
				    if ($active->active == true && $TEMPAT != "" && $TGLLAHIR != "00000000") {
