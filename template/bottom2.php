...
$query='select *   FROM [IDCARD].[dbo].[tb_stock] where qty_limit >= stock';
...
while ($e=odbc_fetch_object($get_unitx)) 
	...
	if($x=='' || $x==NULL)
		...
		if($complain_counter>1)
			{
			$complain_note='there are '.$complain_counter." complain's ";
			}
		else $complain_note='there are '.$complain_counter." complain ";
		}
