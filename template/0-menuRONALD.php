									...
									<li><a href=""><?php echo $word[168]; // Personal Data ?></a>
										<ul>
												...
												<li><a href="MSSplafonmedical.php"><?php echo $word[203]; // Pagu medical ?></a></li>
                                                                        ...
                                                                        <li><a href=""><?php echo "HRP Online"; // Personal Data ?></a>
										<ul>
												<li><a href="MSS_HRPonline.php"><?php echo "HRP Online Request"; // Profile ?></a></li>
												<li><a href="MSS_HRPonline_report.php"><?php echo "HRP Online History"; // Medical by Person ?></a></li>
										</ul>
									</li>
							...
							<ul>
							   ...
							   echo '<li><a href="skklListPendingApprove.php" ><img src="img/icon/alert.png" />'.$word[85].'</a></li>'; //SKKL List Pending Approve
								...
								<?php
								  ...
								  <?php
									$sqlhrss="SELECT COUNT(b.UserLogin) AS JUMLAHSJ FROM ms_ModuleSpecial a,ms_ModuleAdmin b  WHERE a.SpecialName like '%HRSS%' and b.UserLogin='".$NIK."' and a.SpecialID=b.SpecialID and a.SpecialName='HRSS_SJ' ";
										$StatusHrss=odbc_exec($conn,$sqlhrss);
				  			...
				  			//  --------------- SJ (HR SELF SERVICE MENU) END -----------------------
										      ...
										      //  --------------- SPENGANGKATAN (HR SELF SERVICE MENU) START -----------------------?>

								  <?php
									$sqlhrss="SELECT COUNT(b.UserLogin) AS JUMLAHSP FROM ms_ModuleSpecial a,ms_ModuleAdmin b  WHERE a.SpecialName like '%HRSS%' and b.UserLogin='".$NIK."' and a.SpecialID=b.SpecialID and a.SpecialName='HRSS_SPENGANGKATAN' ";
										$StatusHrss=odbc_exec($conn,$sqlhrss);
										if(odbc_result($StatusHrss,"JUMLAHSP")>0)
										{
											?>
											<li><a href=""><!-- <?php echo $word[232]; // Surat Jaminan ?> --> Decree Online </a>
												<ul>
														<li><a href="SuratPengangkatan_Form.php"><!-- <?php echo $word[233]; // ApprovalRequest ?> -->Request Form </a></li>
														<li><a href="SuratPengangkatan_History.php">History Decree</a></li>
														<li><a href="#"><?php echo $word[234]; // MasterData ?></a>
															<ul>
																<li><a href="SuratPengangkatan_MasterData.php"><?php echo $word[238]; ?></a></li>
																<li><a href="SuratPengangkatan_MasterHeader.php"><?php echo $word[239] ?></a></li>
																<li><a href="SuratPengangkatan_MasterTembusan.php">Tembusan</a></li>
															</ul>
														</li>
												</ul>
											</li>
				  			...
				  			//  --------------- SPENGANGKATAN (HR SELF SERVICE MENU) END -----------------------
...
//------------- HRP ONLINE APPROVAL START -------------
//HRP ONLINE MSS APPROVAL MANAGER
$sqlhrponline="select COUNT(*) as jumlahHRP from tb_HRPOnline_transaction where (FirstApproverNIK='$NIK'and isFirstApproved='0') OR (SecondApproverNIK='$NIK' and isSecondApproved='0') OR (ThirdApproverNIK='$NIK' and isThirdApproved='0')
";
$HRPOnline=odbc_exec($conn,$sqlhrponline);
$totalHRPOnline=odbc_result($HRPOnline,"jumlahHRP");

//HRP ONLINE HR APPROVAL
$sqlhrponlineHR2="SELECT COUNT(b.UserLogin) AS JUMLAHhrssHRP, b.PersAdmin as PersAdmin
                    FROM ms_ModuleSpecial a,ms_ModuleAdmin b  
                    WHERE a.SpecialName like '%HRSS%' 
                    and b.UserLogin='$NIK' 
                    and a.SpecialID=b.SpecialID 
                    and (a.SpecialName='HRSS_HRP_ONLINE_APP')
                    GROUP BY b.UserLogin,b.PersAdmin
";
$HRPOnline2=odbc_exec($conn,$sqlhrponlineHR2);
$totalHRPOnlineHR1=odbc_result($HRPOnline2,"JUMLAHhrssHRP");   
$persAdminhrsshrp=odbc_result($HRPOnline2,"PersAdmin");   

if($totalHRPOnlineHR1=='1')
{
    $sqlhrponlineHR3="select COUNT(*) JUMLAHhrssHRP from tb_HRPOnline_transaction where isApproved='1' AND persAdmin='$persAdminhrsshrp' AND isHRApproved='0'
    ";
    $HRPOnline3=odbc_exec($conn,$sqlhrponlineHR3);
    $totalHRPOnlineHR=odbc_result($HRPOnline3,"JUMLAHhrssHRP");   
}

if($totalHRPOnlineHR1=="1")
{
    ?>
<li><a href="">HRP Online</a>
    <ul>
        <li><a href="HRSS_HRPonline_report.php">Report HRP Online</a></li>
    </ul>
</li>
    <?php
}
//------------- HRP ONLINE APPROVAL END -------------
...
?>
	...
	echo "<li><a href='HRSS_ListEmployee.php'>$word[228]</a></li>";
...
//                    ------------------------------------------------------------------------
			...
			<li><a href="kg_university/index.html">KG University</a> </li>
				<!--
				<ul>
				<li><a href="redirect.php?to=forum"><?php //echo $word[48]; ?></a></li>
...
//                ------------------------------------------
                $cekloginpayrollstring2="SELECT COUNT(*) as pa FROM ms_niktelp "
                                        . "where PayArea in ('KA','KB','KC','KX') AND NIK='$NIK'";
                $queryceklogin2 = odbc_exec($conn,$cekloginpayrollstring2);
                $payrolllinkakses2 = odbc_result($queryceklogin2,"pa");
...
//		-------------------------------------------
		...
		<?php
                ...
                <!----------------------- payroll ul end ---------------------------------------->
						...
						<?php
						 $employee_ps="SELECT * FROM ms_EmpOrg WHERE NIK='00".$NIK."'";
                				$qry_ps = odbc_exec($conn,$employee_ps);
                        			$PersAdmin=odbc_result($qry_ps, 'PERSADMIN');
						 $PersArea=odbc_result($qry_ps, 'PERAREA');
                                                 $SubArea=odbc_result($qry_ps, 'SUBAREA');
						...
						<li><a href="#"><?php echo $word[232]; ?></a>
							<ul>
								<?php
									// sql notif start
									$suratJaminan = odbc_exec($conn, "SELECT COUNT(*) as counter FROM s_jaminan a, ms_ModuleAdmin b, ms_ModuleSpecial c WHERE (a.status = '0' AND a.persAdmin = b.PersAdmin AND (c.SpecialName LIKE '%HRSS%' AND b.SpecialID = '87' AND c.SpecialID = b.SpecialID AND (c.SpecialName = 'HRSS_SJ')) AND b.UserLogin = '$NIK')");
						...
						if(odbc_result($rs_user_medrem_admin,"UserLogin")==$NIK)
						{
									...
									$rs_medrem = odbc_exec($conn, "select COUNT(*) AS counter from dbo.tb_mr_trx_medrem_header A inner join tb_mr_trx_medrem_detail B on A.id_header=B.id_header where A.approved_sap is null and persadmin='$PersAdmin'");
							...
							echo "<li><a href='mr_hrs_medrem_status_history.php'>$word[241]</a></li>";
						}
					?>
...
/*			$CekRolesSubAreaPA=odbc_exec($conn,"SELECT * FROM dbo.tb_MotorcycleLoanPA where PersonalArea='$PersArea'
					...
					if (odbc_result($queryTotalHRAppMloan,"total")!=NULL)
					{
						...
						while($row = odbc_fetch_row($queryCountHRAppMloan))
						{
							//validasi personal admin sama dengan personal admin nik yang diminta lembur
						   	$employee_ps="SELECT * FROM ms_EmpOrg WHERE NIK='00".$NIK."'";
                                                	$qry_ps = odbc_exec($conn,$employee_ps);
                                                	$PersAdmin2=odbc_result($qry_ps, 'PERSADMIN');
                                                 	$PersArea2=odbc_result($qry_ps, 'PERAREA');
                                                 	$SubArea2=odbc_result($qry_ps, 'SUBAREA');
				...
				else
				{
					...
					while($row = odbc_fetch_row($resultModulAdminHRmLoan))
					{
						...
						//validasi personal admin sama dengan personal admin nik yang diminta lembur
					   	$employee_ps="SELECT * FROM ms_EmpOrg WHERE NIK='00".$NIK."'";
                                                $qry_ps = odbc_exec($conn,$employee_ps);
                                                $PersAdminHRAdmin=odbc_result($qry_ps, 'PERSADMIN');
                                                 $PersAreaHRAdmin=odbc_result($qry_ps, 'PERAREA');
                                                 $SubAreaHRAdmin=odbc_result($qry_ps, 'SUBAREA');
						...
						while($row = odbc_fetch_row($mLoanAppResultHRAdmin))
						{
							...
							$employee_ps="SELECT * FROM ms_EmpOrg WHERE NIK='00".$NIK."'";
                                                	$qry_ps = odbc_exec($conn,$employee_ps);
                                                	$PersAdminEmployee=odbc_result($qry_ps, 'PERSADMIN');
                                                 	$PersAreaEmployee=odbc_result($qry_ps, 'PERAREA');
                                                 	$SubAreaEmployee=odbc_result($qry_ps, 'SUBAREA');
		...
		<?php 
				// Ini Code Beasiswa
				$queryAccBeasiswa = "SELECT TOP 1 status_daftar,status_test FROM tabel_pendaftaran_beasiswa WHERE NIK = '$NIK' ORDER BY NIS DESC";
				...
				$sqlBeasiswa="SELECT count(*) as totalAdmin FROM ms_ModuleAdmin WHERE ModulAdmin AND UserLogin='$NIK'";
				$queryBeasiswa = odbc_exec($conn,$sqlBeasiswa);
				$totalAdmin = odbc_result($queryBeasiswa,"totalAdmin");

				
				if($totalAdmin>="1" || ($today >= $tgl_mulai && $today <= $tgl_selesai)) {?>
		<li><a href="">Beasiswa</a>
			<ul>
				<?php if($totalAdmin>="1") {?>
					...
					<li><a href="Beasiswa_TambahData_Lama.php">Input Data Siswa Manual</a></li>
				<?php } if($today >= $tgl_mulai && $today <= $tgl_selesai) { ?>
					<li><a href="Beasiswa_RequestForm_Next.php">Pengajuan Beasiswa</a></li>
...
<!--			<li><a href="gov_regulation.php">Undang Undang Pemerintah</a></li> -->
		...
		if ($menuFlag['admin'][2]==1 or $menuFlag['admin'][3]==1 or $menuFlag['admin'][6]==1 or $menuFlag['view'][2]==1 or $menuFlag['view'][3]==1 or $menuFlag['view'][6]==1 or $menuFlag['view'][20]==1){
			...
			echo '<li><a href="http://202.146.0.43:8080/share/page/" target="_blank">CHR Documents</a><li>'; //CHR efiling
	...
	<li><a href=""><?php echo $word[168]; //Personal Data ?></a><ul>
		...
		$Qryadmin=odbc_exec($conn,$sqlAdmin) or die("<p>".odbc_errormsg());
			...
			if(odbc_result($Qryadmin,1)==$NIK)
			{
				$employee_ps="SELECT * FROM ms_EmpOrg WHERE NIK='00".$NIK."'";
                                $qry_ps = odbc_exec($conn,$employee_ps);
                                $PersAdm=odbc_result($qry_ps, 'PERSADMIN');
				$PersArea=odbc_result($qry_ps, 'PERAREA');
